...
class Generator(nn.Module):
    ...
    def stn(self, x):
        ...
        temp = x.shape[3] / 16
        x = x.reshape([int(x.shape[0]), 64, 32, 16, int(temp)])
        arr = []
        for i in range(x.shape[4]):
            xs = self.localization(x[:, :, :, :, i])
            xs = xs.view(-1, 256 * 8 * 4)
            theta = self.fc_loc(xs)
            theta = theta.view(-1, 2, 3)
            grid = F.affine_grid(theta, x[:, :, :, :, i].size())
            x_temp = F.grid_sample(x[:, :, :, :, i], grid)
            arr.append(x_temp)
        arr = torch.stack(arr)
        temp = arr.shape[0]
        arr = arr.reshape([int(x.shape[0]), 64, 32, 16 * int(temp)])
        return arr + 0
    ...
    def forward(self, z, y):
        ...
        # First linear layer
        #print(z.shape) #[8, 14, 2560]
        #Temporarily reshape into [8, 1, 14, 2560]
        h = self.linear(z)
        ...
        # Loop over blocks
        #print(h.shape) #[8, 512, 4, 56]
        
        for index, blocklist in enumerate(self.blocks):
            # Second inner loop in case block has multiple layers
            for block in blocklist:
                h = block(h, ys[index]) #Calls up layers
        # Apply batchnorm-relu-conv-tanh at output
        #print(h.shape) #[8, 64, 32, 224]
        #Original: [64, 1, 28, 28]
