...
class GBlock(nn.Module):
    def __init__(self, in_channels, out_channels,
        ...
        self.fc_loc = nn.Sequential(
            nn.Linear(int(2048 * 512 / in_channels), 32),
            nn.ReLU(True),
            nn.Linear(32, 3 * 2)
        )
    ...
    def stn(self, x):
        ...
        temp = x.shape[3] / x.shape[2]
        x = x.reshape([int(x.shape[0]), self.in_channels, int(x.shape[2]), int(x.shape[2]), int(temp)])
        for i in range(x.shape[4]):
            xs = self.localization(x[:, :, :, :, i])
            xs = xs.view(-1, int(2048 * 512 / self.in_channels))
            theta = self.fc_loc(xs)
            theta = theta.view(-1, 2, 3)
            grid = F.affine_grid(theta, x[:, :, :, :, i].size())
            x[:, :, :, :, i] = F.grid_sample(x[:, :, :, :, i].clone(), grid)
        x = x.reshape([int(x.shape[0]), self.in_channels, int(x.shape[2]), int(x.shape[2]) * int(temp)])
        return x
